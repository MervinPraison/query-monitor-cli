...
define('PRAISON_QMCLI_VERSION', '0.5.0');
...
function praison_qmcli_check_requirements()
{
	if (!class_exists('QueryMonitor')) {
		add_action('admin_notices', function () {
			...
			echo '<strong>' . esc_html__('Praison Debug CLI for Query Monitor:', 'praisonai-cli') . '</strong> ';
			echo esc_html__('Requires Query Monitor plugin to be installed and activated.', 'praisonai-cli');
			echo '</p></div>';
		});
		return false;
	}
